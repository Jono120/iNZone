...
namespace InzoneKioskWebservice
{
	public class TableAdapter
	{
		...
		public static SyncAdapter ParticipantSyncAdapter()
		{
			...
			incInsParticipantCmd.Parameters.Add("@KioskID", SqlDbType.VarChar, 20);
		...
		public static SyncAdapter InteractionSyncAdapter()
		{
			...
			incInsInteractionCmd.CommandText = "SELECT I.ID, I.ParticipantID, I.PartnerID, I.Subscribed, I.DateCreated from [Interaction] AS I " +
								"INNER JOIN [Participant] AS P ON I.ParticipantID = P.ID " +
								   "where I.create_timestamp > @sync_last_received_anchor " +
									 "and I.create_timestamp <= @sync_new_received_anchor " +
									 "and I.update_originator_id <> @sync_client_id_hash " +
			...
			incInsInteractionCmd.Parameters.Add("@KioskID", SqlDbType.VarChar, 20);
			...
			incUpdInteractionCmd.CommandText = "SELECT [Interaction].ID, [Interaction].ParticipantID, [Interaction].PartnerID, [Interaction].Subscribed, [Interaction].DateCreated from [Interaction] " +
										  "INNER JOIN [Participant] ON [Interaction].ParticipantID = [Participant].ID " +
											"where [Interaction].create_timestamp <= @sync_last_received_anchor " +
											"and [Interaction].update_timestamp > @sync_last_received_anchor " +
											"and [Interaction].update_timestamp <= @sync_new_received_anchor	" +
											"and [Interaction].update_originator_id <> @sync_client_id_hash " +
											"and [Participant].KioskID = @KioskID " +
										  "order by [Interaction].update_timestamp desc ";
			...
			incDelInteractionCmd.CommandText = "select [Interaction_tombstone].ID from [Interaction_tombstone] " +
											"INNER JOIN [Participant] ON [Interaction_tombstone].ParticipantID = [Participant].ID " +
										  "where [Interaction_tombstone].update_timestamp > @sync_last_received_anchor " +
											"and [Interaction_tombstone].update_timestamp <= @sync_new_received_anchor " +
											"and [Interaction_tombstone].update_originator_id <> @sync_client_id_hash " +
											"and [Participant].KioskID = @KioskID " +
											"order by [Interaction_tombstone].update_timestamp desc	";
		...
		public static SyncAdapter InteractionVideoSyncAdapter()
		{
			...
			incInsInteractionVideoCmd.CommandText = "SELECT [InteractionVideo].ID, [InteractionVideo].InteractionID, [InteractionVideo].VideoRating, " +
								   "[InteractionVideo].VideoID, [InteractionVideo].VideoName, [InteractionVideo].DateCreated " +
								   "FROM [InteractionVideo] " +
								   "INNER JOIN [Interaction] ON [InteractionVideo].InteractionID = [Interaction].ID " +
								   "INNER JOIN [Participant] ON [Interaction].ParticipantID = [Participant].ID " +
									"where [InteractionVideo].create_timestamp > @sync_last_received_anchor " +
									 "and [InteractionVideo].create_timestamp <= @sync_new_received_anchor " +
									 "and [InteractionVideo].update_originator_id <> @sync_client_id_hash " +
									 "and [Participant].KioskID = @KioskID " +
								   "order by [InteractionVideo].create_timestamp desc ";
			...
			incInsInteractionVideoCmd.Parameters.Add("@KioskID", SqlDbType.VarChar, 20);
			...
			incUpdInteractionVideoCmd.CommandText = "SELECT [InteractionVideo].ID, [InteractionVideo].InteractionID, [InteractionVideo].VideoRating, " +
								   "[InteractionVideo].VideoID, [InteractionVideo].VideoName, [InteractionVideo].DateCreated " +
								   "FROM [InteractionVideo] " +
								   "INNER JOIN [Interaction] ON [InteractionVideo].InteractionID = [Interaction].ID " +
								   "INNER JOIN [Participant] ON [Interaction].ParticipantID = [Participant].ID " +
									  "where [InteractionVideo].create_timestamp <= @sync_last_received_anchor " +
										"and [InteractionVideo].update_timestamp > @sync_last_received_anchor " +
										"and [InteractionVideo].update_timestamp <= @sync_new_received_anchor	" +
										"and [InteractionVideo].update_originator_id <> @sync_client_id_hash " +
										"and [Participant].KioskID = @KioskID " +
										"order by [InteractionVideo].update_timestamp desc ";
			...
			incDelInteractionVideoCmd.CommandText = "select [InteractionVideo_tombstone].ID from [InteractionVideo_tombstone] " +
											"INNER JOIN [Interaction] ON [InteractionVideo_tombstone].InteractionID = [Interaction].ID " +
											"INNER JOIN [Participant] ON [Interaction].ParticipantID = [Participant].ID " +
										  "where [InteractionVideo_tombstone].update_timestamp > @sync_last_received_anchor " +
											"and [InteractionVideo_tombstone].update_timestamp <= @sync_new_received_anchor " +
											"and [InteractionVideo_tombstone].update_originator_id <> @sync_client_id_hash " +
											"and [Participant].KioskID = @KioskID " +
											"order by [InteractionVideo_tombstone].update_timestamp desc	";
		...
		public static SyncAdapter BadWordFilterSyncAdapter()
		{
			// BadWordFilter Table
			SyncAdapter adapterBadWordFilter = new SyncAdapter("BadWordFilter");

			// select incremental inserts command
			SqlCommand incInsBadWordFilterCmd = new SqlCommand();
			incInsBadWordFilterCmd.CommandType = CommandType.Text;
			incInsBadWordFilterCmd.CommandText = "SELECT ID, BadWord, UseWildCard, DateCreated " +
								   "FROM [BadWordFilter] " +
									"where create_timestamp > @sync_last_received_anchor " +
									 "and create_timestamp <= @sync_new_received_anchor " +
									 "and update_originator_id <> @sync_client_id_hash " +
								   "order by create_timestamp desc ";
			incInsBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncClientIdHash, SqlDbType.Int);
			incInsBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncLastReceivedAnchor, SqlDbType.Binary, 8);
			incInsBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncNewReceivedAnchor, SqlDbType.Binary, 8);

			adapterBadWordFilter.SelectIncrementalInsertsCommand = incInsBadWordFilterCmd;

			// select incremental updates command
			SqlCommand incUpdBadWordFilterCmd = incInsBadWordFilterCmd.Clone();
			incUpdBadWordFilterCmd.CommandText = "SELECT ID, BadWord, UseWildCard, DateCreated " +
								   "FROM [BadWordFilter] " +
									  "where create_timestamp <= @sync_last_received_anchor " +
											"and update_timestamp > @sync_last_received_anchor " +
											"and update_timestamp <= @sync_new_received_anchor	" +
			...
			adapterBadWordFilter.SelectIncrementalUpdatesCommand = incUpdBadWordFilterCmd;

			// select incremental deletes command
			SqlCommand incDelBadWordFilterCmd = incInsBadWordFilterCmd.Clone();
			incDelBadWordFilterCmd.CommandText = "select ID from [BadWordFilter_tombstone] " +
										  "where update_timestamp > @sync_last_received_anchor " +
											"and update_timestamp <= @sync_new_received_anchor " +
											"and update_originator_id <> @sync_client_id_hash " +
											"order by update_timestamp desc	";

			adapterBadWordFilter.SelectIncrementalDeletesCommand = incDelBadWordFilterCmd;

			// insert row com
			SqlCommand insBadWordFilterCmd = new SqlCommand();
			insBadWordFilterCmd.CommandType = CommandType.StoredProcedure;
			insBadWordFilterCmd.CommandText = "usp_BadWordFilter_applyinsert";

			insBadWordFilterCmd.Parameters.Add("@ID", SqlDbType.Int);
			insBadWordFilterCmd.Parameters.Add("@BadWord", SqlDbType.VarChar, 50);
			insBadWordFilterCmd.Parameters.Add("@UseWildCard", SqlDbType.Bit);
			insBadWordFilterCmd.Parameters.Add("@DateCreated", SqlDbType.SmallDateTime);

			insBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncClientIdHash, SqlDbType.Int);
			insBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncLastReceivedAnchor, SqlDbType.Binary, 8);
			insBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncRowCount, SqlDbType.Int).Direction = ParameterDirection.Output;

			//adapterBadWordFilter.InsertCommand = insBadWordFilterCmd;


			// update row command
			SqlCommand updBadWordFilterCmd = new SqlCommand();
			updBadWordFilterCmd.CommandType = CommandType.StoredProcedure;
			updBadWordFilterCmd.CommandText = "usp_BadWordFilter_applyupdate";

			updBadWordFilterCmd.Parameters.Add("@ID", SqlDbType.Int);
			updBadWordFilterCmd.Parameters.Add("@BadWord", SqlDbType.VarChar, 50);
			updBadWordFilterCmd.Parameters.Add("@UseWildCard", SqlDbType.Bit);
			updBadWordFilterCmd.Parameters.Add("@DateCreated", SqlDbType.SmallDateTime);

			updBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncClientIdHash, SqlDbType.Int);
			updBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncLastReceivedAnchor, SqlDbType.Binary, 8);
			updBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncRowCount, SqlDbType.Int).Direction = ParameterDirection.Output;

			//adapterBadWordFilter.UpdateCommand = updBadWordFilterCmd;

			// delete row command
			SqlCommand delBadWordFilterCmd = new SqlCommand();
			delBadWordFilterCmd.CommandType = CommandType.StoredProcedure;

			delBadWordFilterCmd.CommandText = "usp_BadWordFilter_applydelete";
			delBadWordFilterCmd.Parameters.Add("@ID", SqlDbType.Int);

			delBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncClientIdHash, SqlDbType.Int);
			delBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncLastReceivedAnchor, SqlDbType.Binary, 8);
			delBadWordFilterCmd.Parameters.Add("@" + SyncSession.SyncRowCount, SqlDbType.Int).Direction = ParameterDirection.Output;

			//adapterBadWordFilter.DeleteCommand = delBadWordFilterCmd;

			// get update conflicting row command
			SqlCommand updcftBadWordFilterCmd = new SqlCommand();
			updcftBadWordFilterCmd.CommandType = CommandType.StoredProcedure;
			updcftBadWordFilterCmd.CommandText = "usp_BadWordFilter_getupdateconflict";
			updcftBadWordFilterCmd.Parameters.Add("@ID", SqlDbType.Int);

			adapterBadWordFilter.SelectConflictUpdatedRowsCommand = updcftBadWordFilterCmd;

			// get delete conflicting row command
			SqlCommand delcftBadWordFilterCmd = new SqlCommand();
			delcftBadWordFilterCmd.CommandType = CommandType.StoredProcedure;
			delcftBadWordFilterCmd.CommandText = "usp_BadWordFilter_getdeleteconflict";
			delcftBadWordFilterCmd.Parameters.Add("@ID", SqlDbType.Int);

			adapterBadWordFilter.SelectConflictDeletedRowsCommand = delcftBadWordFilterCmd;

			return adapterBadWordFilter;
		}
	}
}
