	...
	public partial class InZoneMailService: ServiceBase {
		...
		protected override void OnStart(string[] args) {
			try {
				...
				try {
					db = new InZoneDAL(Properties.Settings.Default.InzoneMirrorConnectionString);
			...
			try {
				...
				foreach (Mailout mo in query) {
					...
					try {
						...
						XmlDocument serviceParams = getServiceParameters(mo.ServiceID, mo.CustomerID);
						...
						subject = subject.Replace("@CUSTOMER@", mo.Customer.DisplayName);
						...
						bodyHtml = bodyHtml.Replace("@CUSTOMER@", mo.Customer.DisplayName);
						bodyHtml = bodyHtml.Replace("@REDIRECT@",
							"http://www.inzone.co.nz/mail/redirect?" + Webstream.UrlEncoding.Base64UrlEncode(mo.ID.ToString()));
						...
						bodyHtml = bodyHtml.Replace("@UNSUBSCRIBE_LINK@",
							"http://www.inzone.co.nz/mail/unsubscribe?" + Webstream.UrlEncoding.Base64UrlEncode(mo.ID.ToString()));
						bodyHtml = bodyHtml.Replace("@CUSTOMER_LOGO@", mo.Customer.LogoURL);
