...
namespace InzoneKioskWebservice
{
    ...
    public class InzoneKioskWebService : System.Web.Services.WebService
    {
        ...
        /// <summary>
        /// The KioskID for accessing the method GetDataSet
        /// </summary>
        private const string GetDataSetKioskID = "SpeicalKioskID";
        ...
        private void Authenticate()
        {
            ...
            if (kioskID == GetDataSetKioskID && Authentication.Password != ConfigurationManager.AppSettings["GetDataSetPassword"])
            {
                // Kiosk ID is for the one specific to accessing the method GetDataSet
                // So we read in the password from the config file
            ...
            if (kioskID != GetDataSetKioskID && Authentication.Password != EncodePassword(kioskID))
            {
                EventLogWriter.WriteToLogFile("Invalid password information.  Unable to execute web method.");
                throw new Exception("Invalid password information.  Unable to execute web method.");
            }
        ...
        /// <summary>
        /// Runs query against a database
        /// </summary>
        /// <param name="query">The query string to run against the database</param>
        /// <returns>Result of running the query string, represented in a Dataset</returns>
        [SoapHeader("Authentication")]
        [WebMethod]
        public DataSet GetDataSet(string query)
        {
            // Autherticate this request
            Authenticate();

            // Connect to database using connection string with read only access
            SqlConnection databaseConnection = new SqlConnection(ConfigurationManager.AppSettings["ReadOnlyConnectionString"]);
